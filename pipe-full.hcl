...
# 更改向后选择分支（BTFNT），增加分支预测准确性：
# 	（1）f_pc：如果预测错误，改变预测；
# 	（2）f_predPC：如果是条件转移（也不是jmp），就向地址小的分支转移；
...
wordsig D_ifun 'if_id_curr->ifun'	# 译码阶段的ifun
wordsig D_valC 'if_id_curr->valc'	# 译码阶段的valC
...
word f_pc = [
	...
	# 新增BTFNT转发（以下是分支预测错误后的转发）：
	# M_valE中存的是valC，M_valA中存的是增加后的PC(valP).
	# M_cnd表示是否跳转.
...
word f_predPC = [
	# Add BTFNT prediction:
	f_icode == IJXX && f_ifun != 0 && f_valC < f_valP : f_valC;
	f_icode == IJXX && f_ifun != 0 && f_valC > f_valP : f_valP;


	f_icode in { IJXX, ICALL } : f_valC;	# Always taken.
	1 : f_valP;
];
...
word d_valA = [
	# 设法判断运行环境.
	D_icode == IOPQ && D_ifun == 2 && D_rA == 12 : 1;
...
word d_valB = [
	...
	# 设法判断运行环境.
	D_icode == IOPQ && D_ifun == 2 && D_rB == 12 : 1;
...
word aluA = [
	...
	E_icode == IIADDQ &&  E_valC == 1 && M_valE <= 0  : 0;
...
bool F_stall =
	...
	# 改为加载转发后需要对F_stall加以修改.
	# 加上加载转发之后，去掉 E_dstM == d_srcA时指令是IIRMOVQ或IPUSHQ的暂停要求.
	E_icode in { IMRMOVQ, IPOPQ } &&
	(
...
bool D_stall = 
	...
	# 改为加载转发后需要对D_stall加以修改.
	# 加上加载转发之后，去掉 E_dstM == d_srcA 时，指令是IIRMOVQ或IPUSHQ的暂停要求.
	E_icode in { IMRMOVQ, IPOPQ } &&
	(
		E_dstM == d_srcB || ( E_dstM == d_srcA && !D_icode in { IRMMOVQ, IPUSHQ })
	);
